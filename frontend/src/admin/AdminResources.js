...
import { PiCheckCircleDuotone, PiXCircleDuotone, PiTrashDuotone, PiTrayDuotone, PiLinkDuotone, PiFileTextDuotone, PiUserDuotone, PiBookOpenDuotone, PiPackageDuotone, PiClockDuotone, PiEyeDuotone, PiShieldCheckDuotone, PiDownloadSimpleDuotone, PiCursorClickDuotone, PiSealCheckFill } from 'react-icons/pi';
import { LanguageContext } from '../LanguageContext';
import { getApiUrl } from '../api';

export default function AdminResources({ setPage, userFilter }) {
  ...
  async function reject(id) {
    try {
      ...
      if (!res.ok) throw new Error();
      showToast(t("تم رفض المورد"), C.red);
      fetchResources(meta.current_page);
    } catch (err) { showToast(t("فشل الإجراء"), C.red); }
  }

  async function remove(id) {
    if (!window.confirm(t("هل أنت متأكد من حذف هذا المورد نهائياً؟"))) return;
    try {
    ...
    <>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28 }}>
        <div>
          ...
          <p style={{ color:C.muted, fontSize:"0.88rem" }}>{t("مراجعة وإدارة جميع الموارد المرفوعة في النظام")}</p>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:22 }}>
        {[[t("all"),t("الكل")],[t("pending"),t("قيد المراجعة")],[t("approved"),t("مقبول")]].map(([id,l])=>(
          <div key={id} onClick={()=>setFilter(id)} style={{ padding:"7px 18px", borderRadius:20, cursor:"pointer", fontSize:"0.86rem", fontWeight:filter===id?700:400, background:filter===id?C.blue:C.white, color:filter===id?C.white:C.muted, border:`1.5px solid ${filter===id?C.blue:C.border}`, transition:"all .2s" }}>
            {l}
          </div>
        ))}
      </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:'blur(4px)' }} onClick={()=>setPreviewFile(null)}>
          <div style={{ background:C.white, borderRadius:24, width:800, maxWidth:"95%", height:'85vh', boxShadow:"0 25px 50px rgba(0,0,0,0.2)", position:'relative', display:'flex', flexDirection:'column', overflow:'hidden' }} onClick={e=>e.stopPropagation()}>
            <div style={{ padding:'20px 28px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center', background:C.bg }}>
              <div>
                <h2 style={{ fontSize:'1.1rem', fontWeight:800, color:C.dark, margin:0 }}>{t("معاينة المورد: ")}{previewFile.title}</h2>
            ...
            <div style={{ flex:1, background:"#f8f9fa", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
              {previewFile.resource_type === 2 ? (
                <div style={{ textAlign:"center", padding:40 }}>
                  ...
                  <p style={{ color:C.muted, marginBottom:24 }}>{previewFile.file_url}</p>
                  <Btn onClick={()=>window.open(previewFile.file_url, '_blank')}>{t("فتح الرابط في علامة تبويب جديدة")}</Btn>
                </div>
              ) : (
                <div style={{ width:"100%", height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
                   {previewFile.mime_type?.startsWith('image/') ? (
                     <img src={`${apiUrl}/api/resources/view/${previewFile.uuid}`} style={{ maxWidth:'100%', maxHeight:'100%', objectFit:'contain' }} alt="preview" />
                   ) : (
                     <iframe 
                       src={`${apiUrl}/api/resources/view/${previewFile.uuid}`} 
            ...
            <div style={{ padding:'16px 28px', borderTop:`1px solid ${C.border}`, display:'flex', justifyContent:'flex-end', gap:10, background:C.bg }}>
               ...
               {previewFile.is_approved === false && (
                 <>
                   <Btn variant="danger" style={{ padding:'8px 24px' }} onClick={()=>{ reject(previewFile.id); setPreviewFile(null); }}>{t("رفض المورد")}</Btn>
                   <Btn variant="success" style={{ padding:'8px 24px' }} onClick={()=>{ approve(previewFile.id); setPreviewFile(null); }}>{t("قبول المورد")}</Btn>
                 </>
               )}
               {previewFile.is_approved === true && (
                 <>
                    <Btn variant="secondary" style={{ display:'flex', alignItems:'center', gap:6, color:C.red, border:`1px solid ${C.red}30` }} onClick={()=>{ reject(previewFile.id); setPreviewFile(null); }}><PiXCircleDuotone/> {t("سحب القبول")}</Btn>
                    <Btn variant="primary" style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 24px' }} onClick={()=>{
                      alert(t("بدأ تحميل الملف..."));
